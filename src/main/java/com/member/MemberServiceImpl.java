...
//        }
		...
		if (member == null || 
			...
			member.getMemberStatus() == 0) {
			throw new RuntimeException(member == null || !memberPassword.equals(member.getMemberPassword()) ? 
				"使用者名稱或密碼不正確" : 
				"此帳號尚未完成Email驗證，請先至信箱點擊驗證連結");
		}
	...
	public MemberDTO updateMember(Integer memberNo, MemberDTO memberDTO) {
		Member member = memberRepository.findById(memberNo).orElseThrow(() -> new EntityNotFoundException("會員不存在"));
	 ...
	 * 
	 * @Override public List<MemberDTO> findMembersByName(String name) { return
	 * memberRepository.findByMemberNameContaining(name).stream()
	 * .map(MemberDTO::fromEntity) .collect(Collectors.toList()); }
	 */

	@Override
	public List<MemberDTO> searchMembers(MemberSearchDTO searchDTO) {
		List<Member> members = memberRepository.findAll(MemberSpecifications.withSearchCriteria(searchDTO));
		return members.stream().map(MemberDTO::fromEntity).collect(Collectors.toList());
	}

	@Override
	public Page<MemberDTO> searchMembersWithPagination(MemberSearchDTO searchDTO, Pageable pageable) {
		Page<Member> membersPage = memberRepository.findAll(MemberSpecifications.withSearchCriteria(searchDTO),
				pageable);
	...
	public void updateMemberPhoto(Integer memberNo, MultipartFile photo) {
		...
		try {
			...
			StreamingData streamingData = new StreamingData(photo.getInputStream(), photo.getSize(),
	...
	public StreamingData getMemberPhoto(Integer memberNo) {
		...
		try {
			...
			return new StreamingData(new ByteArrayInputStream(member.getMemberPhoto()), member.getMemberPhoto().length,
					contentType);
		} catch (IOException e) {
			throw new RuntimeException("處理照片時發生錯誤", e);
		}
	...
	private String determineImageContentType(byte[] imageData) throws IOException {
		...
		if (imageData[0] == (byte) 0xFF && imageData[1] == (byte) 0xD8) {
			...
			return "image/gif";
		} else if (new String(imageData, 0, 5).equals("<?xml") || new String(imageData, 0, 4).equals("<svg")) {
	...
	public List<MemberDTO> getAllMembers() {
		return memberRepository.findAll().stream().map(MemberDTO::fromEntity).collect(Collectors.toList());
	}
